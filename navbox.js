let bottomBoxes = [];

bottomBoxes.push({
	heading: 'Cardiacs Songs',
	categories: [{
		category: 'The Early Days',
		info: [{
			name: '1978 Demo',
			info: [
				'When I See That Certain Look In Your Eye',
				'I Bit The Vicar',
				'Vic Bite 3a',
				'Minnow Bandage',
				'Pilf',
				'Ants',
				'Trade Mark'
			]
		}, {
			name: 'Cardiac Arrest EP',
			info: [
				'<a href="./index.html?s=bus-bus">A Bus for a Bus on the Bus</a>',
				'<a href="./index.html?s=bus-cake">A Cake for Bertie\'s Party</a>',
				'<a href="./index.html?s=bus-food">Food on the Wall</a>',
				'Keep Your Dead Mice With You'
			]
		}, {
			name: 'The Obvious Identity',
			info: [
				'songs here',
			]
		}, {
			name: 'Toy World',
			info: [
				'<a href="./index.html?s=toy-aukamacic">Aukamacic</a>',
				'<a href="./index.html?s=toy-qualms">Icky Qualms</a>',
				'Over + Over + Over + Over',
				'Dead Mouse',
				'A Big Noise in a Toy World',
				'Trademark',
				'<a href="./index.html?s=toy-scrawl">Scratching Crawling Scrawling</a>',
				'<a href="./index.html?s=toy-sea">As Cold As Can Be In An English Sea</a>',
				'<a href="./index.html?s=toy-life">Is This the Life?</a>',
				'Nurses Whispering Verses',
				'A Time For Rejoicing',
			]
		}, {
			name: 'The Seaside',
			info: [
				'<a href="./index.html?s=seaside-jibber">Jibber And Twitch</a>',
				'<a href="./index.html?s=seaside-jina">Gina Lollobrigida</a>',
				'<a href="./index.html?s=seaside-sparrow">Hello Mr. Sparrow</a>',
				'<a href="./index.html?s=seaside-lovelyday">It\'s a Lovely Day</a>',
				'<a href="./index.html?s=seaside-wood">A Wooden Fish on Wheels</a>',
				'<a href="./index.html?s=seaside-nurses">Nurses Whispering Verses</a>',
				'<a href="./index.html?s=seaside-life">A Little Man and a House</a>',
				'<a href="./index.html?s=seaside-littleManAndAHouse">A Little Man and a House</a>',
				'<a href="./index.html?s=seaside-hopeDay">Hope Day</a>',
				'<a href="./index.html?s=seaside-dinner">Dinner Time</a>',
				'<a href="./index.html?s=seaside-ice">Ice a Spot and a Dot on the Dog</a>',
				'<a href="./index.html?s=seaside-res">R.E.S.</a>',
				'<a href="./index.html?s=seaside-toGoOffAndThings">To Go Off and Things</a>',
			]
		}]
	}, {
		category: 'The Sextet Era',
		info: [{
			name: 'A Little Man and a House and the Whole World Window',
			info: [
				'songs here',
			]
		}, {
			name: 'On Land And In The Sea',
			info: [
				'songs here',
			]
		}, {
			name: 'Archive Cardiacs',
			info: [
				'<a href="./index.html?s=toy-aukamacic">Aukamacic</a>',
				'<a href="./index.html?s=toy-qualms">Icky Qualms</a>',
				'<a href="./index.html?s=archives-piffol4">Piffol Four Times</a>',
				'<a href="./index.html?s=toy-scrawl">Scratching Crawling Scrawling</a>',
				'<a href="./index.html?s=toy-sea">As Cold As Can Be In an English Sea</a>',
				'<a href="./index.html?s=archives-tvtv">T.V.T.V.</a>',
				'My Trade Mark',
				'The Obvious Identity',
				'<a href="./index.html?s=archives-piffol1">Piffol One Time</a>',
				'A Game for Bertie\'s Party',
				'<a href="./index.html?s=archives-piffol3">Piffol Three Times</a>',
				'Rock Around the Clock'
			]
		}, {
			name: 'Songs For Ships And Irons',
			info: [
				'Big Ship',
				'Tarred and Feathered',
				'Burn Your House Brown',
				'Stoneage Dinosaurs',
				'Plane Plane Against the Grain',
				'Everything Is Easy',
				'There\'s Too Many Irons In the Fire',
				'All Spectacular',
				'<a href="./index.html?s=sfsai-blind">Blind In Safety and Leafy In Love</a>',
				'Loosefish Scapegrace',
				'All His Geese Are Swans'
			]
		}]
	}, {
		category: 'The Later Days',
		info: [{
			name: 'Day Is Gone',
			info: [
				'songs here'
			]
		}, {
			name: 'Heaven Born and Ever Bright',
			info: [
				'songs here'
			]
		}, {
			name: 'Odd Even',
			info: [
				'songs here',
			]
		}, {
			name: 'Manhoo',
			info: [
				'songs here',
			]
		}, {
			name: 'Bell Stinks/Bell Clinks',
			info: [
				'Unknown',
			]
		}, {
			name: 'Dirty Boy',
			info: [
				'Unknown',
			]
		}, {
			name: 'Sing to God',
			info: [
				'<a href="./index.html?s=stg-eden">Eden on the Air</a>',
				'<a href="./index.html?s=stg-wormsHero">Eat It Up Worms Hero</a>',
				'<a href="./index.html?s=stg-sparky">Dog-Like Sparky</a>',
				'<a href="./index.html?s=stg-fieryGunHand">Fiery Gun Hand</a>',
				'<a href="./index.html?s=stg-lassie">Insect Hooves On Lassie</a>',
				'<a href="./index.html?s=stg-fairyMaryMag">Fairy Mary Mag</a>',
				'<a href="./index.html?s=stg-bellyeye">Bellyeye</a>',
				'<a href="./index.html?s=stg-horsesTail">A Horses Tail</a>',
				'<a href="./index.html?s=stg-manhoo">Manhoo</a>',
				'<a href="./index.html?s=stg-wireless">Wireless</a>',
				'<a href="./index.html?s=stg-dirtyBoy">Dirty Boy</a>',
				'<a href="./index.html?s=stg-billion">Billion</a>',
				'<a href="./index.html?s=stg-oddEven">Odd Even</a>',
			]
		}, {
			name: 'Guns',
			info: [
				'songs here',
			]
		}, {
			name: 'Signs',
			info: [
				'songs here',
			]
		}, {
			name: 'Ditzy Scene',
			info: [
				'songs here',
			]
		}, {
			name: 'Vermin Mangle',
			info: [
				'songs here',
			]
		}]
	}, {
		category: 'Going deeper...',
		info: [{
			name: 'Unreleased Content',
			info: [
				"A Picture Of Misery",
				"Bits and Bobs",
				"The Bitter Pill",
				"Daddy Wouldn\'t Buy Me A Wheelchair",
				"Hymn",
				"I Haven\'t Done Anything Yet",
				"Lemmings",
				"London",
				"The New General",
				'<a href="./index.html?s=misc-piffol5">Piffol Five Times</a>',
				'<a href="./index.html?s=misc-piffol6">Piffol Six Times</a>',
				"Protocol",
				'<a href="./index.html?s=misc-silveryRadioSession">Silvery</a>',
				"Time and Space"
			]
		}]
	}]
});
